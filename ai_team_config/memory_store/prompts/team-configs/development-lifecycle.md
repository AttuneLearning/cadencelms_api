...
**Version:** 1.0.0
**Created:** 2026-02-05
**ADR:** [[../../dev_communication/shared/architecture/decisions/ADR-DEV-002-DEVELOPMENT-LIFECYCLE]]
...
context:
  steps:
    ...
    - action: load_memory
      ...
      command: |
        - Search {memory_root}/patterns/ for related patterns
        - Search {memory_root}/entities/ for related entities
        - Search {memory_root}/context/ for domain knowledge
...
implementation:
  steps:
    ...
    - action: implement
      ...
      constraints:
        - "Follow patterns from {memory_root}"
...
completion:
  steps:
    ...
    - action: store_session
      ...
      command: |
        Create {memory_root}/sessions/{date}-{issue-slug}.md with:
        - Issue ID and title
...
on_error:
  ...
  context_compaction:
    ...
    template: "{memory_root}/templates/session-template.md"
```
...
User: "/develop API-ISS-022"
→ Processes single issue with full verification

User: "/develop quick src/services/userService.ts"
